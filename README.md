...
The first step is to set up the Virtual Clusters and Deployments.
You can do that by applying the [full-example-setup.yaml](samples/full-example-setup.yaml) with the command:
...
Now you can set up a Gateway to expose the Service you just created to the outside. For this example it is necessary to define host names for the gateways we are going to create.
...
To do that, use the following configuration. **Take care to specify one of the host names you defined in the Gateway and not the one used in the example**

![](images/screen33.png)

Shortly after submitting you will be able to access the Service with the first one of the host names you defined.
Let's map also the other host name, using a second Virtual Service.

![](images/screen34.png)

If you try now calling the second host name you will be sent to subset2 in the Service you created.
You now are able to access two different subsets of your Service based on the host name you use.

This was easy enough.
Let's now try something different.
Go back to List Virtual Cluster and select vamp-test2.
This time we want to create a single Virtual Service that will respond to a single host name to the outside world and that will dispatch requests to either Service svc-1 or a new Service based on their url while at the same time rewriting that url.
...
In order to do that you need to define two conditional routes with the following conditions:
...
Also worthy of note is the fact that, since svc-1 resides in a different Virtual Cluster, we have to use its fully qualified name to be able to reach it, that is svc-1.vamp-test1.svc.cluster.local.
Fo reference you can find the full configuration below.
...
In this Virtual Cluster, as shown in the initial graph, we want to allow access to Gateway gw-2 from outside the Cluster.
...
Thanks to the Service Entry you can now also add a Virtual Service definition that references the externa service and use it to define timeouts or retry policies as shown in the image.
